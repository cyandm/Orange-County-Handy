...
<div id="reviews-load-more" class="container flex justify-center" <?php echo $oob ? 'hx-swap-oob="true"' : ''; ?>>
	<?php if ($next_url) : ?>
		<button type="button" class="primary-btn group disabled:cursor-wait" hx-get="<?php echo esc_url($next_url); ?>" hx-target="#reviews-grid" hx-swap="beforeend" hx-disabled-elt="this">
			<span class="group-disabled:hidden">
				<?php esc_html_e('load more reviews', 'orange-county-handy'); ?>
			</span>
